...
var easyuiErrorFunction = function(XMLHttpRequest) {
	$.messager.progress('close');
	//var err=XMLHttpRequest.responseText.substring(0,100);
	//alert(XMLHttpRequest.responseText)
	if(XMLHttpRequest==""||XMLHttpRequest=='undefined'||XMLHttpRequest.responseText==""){
		$.messager.alert('系统提示:', '您操作已超时，请重新登陆');
	}else{
		$.messager.alert('系统提示:', XMLHttpRequest.responseText);
	}

	// window.open('http://www.baidu.com',"_blank",'width=310,height=250,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no');
};
...
$.ajaxSetup({
	...
	error : function(XMLHttpRequest, textStatus, errorThrown) {
		...
		if(XMLHttpRequest==""||XMLHttpRequest=='undefined'||XMLHttpRequest.responseText==""){
			...
			$.messager.alert('系统异常:', XMLHttpRequest.responseText);
		}
	}
});
